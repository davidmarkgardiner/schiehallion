    ...
    return (
      <div
        className={`
          ...
          ${isSelected
            ? 'border-lundies-heather bg-lundies-heather/10 shadow-lg shadow-lundies-heather/20'
      ...
      >
        {/* Selection indicator */}
        {isSelected && (
          <div className="absolute -top-2 -right-2 w-6 h-6 rounded-full bg-lundies-heather flex items-center justify-center">
            <svg className="w-4 h-4 text-lundies-charcoal" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div className="mb-4">
          ...
          <p className="text-lundies-stone text-sm leading-relaxed">
        ...
        <div className="mb-4">
          <div className="flex items-baseline gap-2 mb-1">
            <span className="text-2xl font-bold text-lundies-heather">
              {formatPrice(total)}
            </span>
            <span className="text-lundies-stone text-sm">
              total ({numberOfNights} nights)
            </span>
          </div>

          {packageOption.priceAdjustment > 0 && (
            <div className="text-sm text-lundies-stone">
              {formatPrice(basePrice)} room + {formatPrice(packageOption.priceAdjustment)}/night package
            </div>
          )}

          {savings > 0 && (
            <div className="text-sm text-lundies-heather font-medium">
        ...
        {/* Includes */}
        <div className="mb-4">
          <h4 className="text-sm font-medium text-lundies-linen mb-2">Includes:</h4>
          <ul className="space-y-1">
            {packageOption.includes.map((item, index) => (
              <li key={`${packageOption.type}-include-${index}`} className="text-sm text-lundies-stone flex items-start gap-2">
                <span className="text-lundies-heather mt-1">•</span>
                {item}
              </li>
            ))}
          </ul>
        ...
        {packageOption.mealTimes && (
          <div className="mb-4">
            <h4 className="text-sm font-medium text-lundies-linen mb-2">Meal Times:</h4>
            <div className="space-y-1">
              {Object.entries(packageOption.mealTimes).map(([meal, time]) => (
                <div key={meal} className="flex justify-between text-sm">
                  <span className="text-lundies-stone capitalize">{meal}:</span>
        ...
        {hasMenu && (
          <button
            ...
            className="w-full mt-4 py-2 text-sm text-lundies-heather hover:text-lundies-heather/80 transition-colors border-t border-white/10 pt-4"
          >
            View sample menu →
          </button>
        )}
    ...
    <div className="space-y-6">
      ...
      {showTerms && (
        <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
          ...
          <div className="space-y-3 text-sm text-lundies-stone max-h-40 overflow-y-auto">
            <p>
              <strong className="text-lundies-linen">Booking Terms:</strong> All bookings are subject to availability and confirmation.
              Room rates include VAT at the current rate.
            </p>

            <p>
              <strong className="text-lundies-linen">Cancellation Policy:</strong> Free cancellation up to 48 hours before arrival.
              Cancellations within 48 hours may incur a charge equivalent to one night&apos;s stay.
            </p>

            <p>
              <strong className="text-lundies-linen">Package Terms:</strong> Meal packages are non-transferable and must be used during your stay.
              Dietary requirements should be notified at least 24 hours in advance.
            </p>

            <p>
              <strong className="text-lundies-linen">Check-in/Check-out:</strong> Check-in from 3:00 PM, check-out by 11:00 AM.
              Early check-in and late check-out may be available subject to availability and additional charges.
            </p>

            <p>
              <strong className="text-lundies-linen">Payment:</strong> Payment is required at the time of booking.
          ...
          <div className="mt-4 flex items-start gap-3">
            <input
              ...
              className="mt-1 w-4 h-4 rounded border-white/20 bg-black/30 text-lundies-heather focus:ring-lundies-heather focus:ring-2"
            />
            <label htmlFor="terms-checkbox" className="text-sm text-lundies-linen cursor-pointer">
          ...
          {onTermsAccept && (
            <button
              ...
              className="w-full mt-4 py-3 rounded-full bg-lundies-heather text-lundies-charcoal font-semibold hover:bg-lundies-heather/80 transition disabled:opacity-50 disabled:cursor-not-allowed"
            >
      ...
      {showMenuPreview && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="max-w-md w-full max-h-[80vh] overflow-y-auto rounded-3xl border border-white/10 bg-lundies-charcoal/95 backdrop-blur-sm p-6">
            ...
            <p className="text-lundies-stone text-sm mb-4">
            ...
            <ul className="space-y-2">
              {menuPreviewData[showMenuPreview]?.items.map((item: string, index: number) => (
                <li key={`${showMenuPreview}-item-${index}`} className="text-sm text-lundies-linen flex items-start gap-2">
                  <span className="text-lundies-heather mt-1">•</span>
                  {item}
                </li>
              )) || []}
            ...
            <div className="mt-6">
              <button
                onClick={() => setShowMenuPreview(null)}
                className="w-full py-3 rounded-full border border-white/20 text-lundies-linen hover:bg-white/5 transition"
              >
                Close Preview
              </button>
            </div>
